import React from 'react'
const data = {

// PAGE TITLES AND QUOTES
    "profileTitleEn": "Profile",
    "profileTitleFr": "Profil",
    "profileTitleGr": "Profil",

    "profileSubtitleEn": "I'm an, unusual, economist and javascript webdeveloper",
    "profileSubtitleFr": "Je suis un économiste et un développeur javascript inaccoutumé",
    "profileSubtitleGr": "Ich bin ein ungewöhnlicher Wirtschaftler und Javascript-Webentwickler",
    
    "experienceTitleEn": "Experience",
    "experienceTitleFr": "Expérience",
    "experienceTitleGr": "Erfahrung",

    "experienceQuoteEn": <p>Don't be afraid to give up the good to go for the great</p>,
    "experienceQuoteFr": <p>n'ayez pas peur d'abandonner le bien pour le meilleur</p>,
    "experienceQuoteGr": <p>Hab keine Angst, das Gute für die Großen aufzugeben</p>,

    "abilitiesTitleEn": "Abilities",
    "abilitiesTitleFr": "Compétences",
    "abilitiesTitleGr": "Können",

    "abilitiesQuoteEn": <p>Everybody should learn to program a computer, because it teaches you how to think</p>,
    "abilitiesQuoteFr": <p>Tout le monde devrait apprendre à programmer un ordinateur, car il nous apprend comment réfléchir</p>,
    "abilitiesQuoteGr": <p>Jeder sollte lernen, einen Computer zu programmieren, weil er Ihnen das Denken beibringt</p>,

    "projectsTitleEn": "Projects",
    "projectsTitleFr": "Projets",
    "projectsTitleGr": "Projekte",

    "projectsQuoteEn": <p>You don't drown by falling in the water; you drown by staying there</p>,
    "projectsQuoteFr": <p>Tu ne te noies pas en tombant dans l'eau; tu te noies en restant sur place</p>,
    "projectsQuoteGr": <p>Sie ertrinken nicht, indem Sie ins Wasser fallen. Sie ertrinken, während Sie dort bleiben</p>,

    "interestTitleEn": "Points of interest",
    "interestTitleFr": "Centres d'intérêt",
    "interestTitleGr": "Intersse",

    "interestQuoteEn": <p>Leisure is the time for doing something useful. This leisure the diligent person will obtain the lazy one never</p>,
    "interestQuoteFr": <p>Le loisir est le moment de faire quelque chose d'utile. Ce loisir la personne diligente obtiendra, la paresseuse jamais</p>,
    "interestQuoteGr": <p>Freizeit ist die Zeit, um etwas Nützliches zu tun. Diese Freizeit wird die fleißige Person bekommen, die faul nie</p>,

    "footerTitleEn": "Contact",
    "footerTitleFr": "Contact",
    "footerTitleGr": "Kontakt",

    "footerQuoteEn": <p>Coming together is a beginning, staying together is progress, and working together is success</p>,
    "footerQuoteFr": <p>Se réunir est un début, rester ensemble est le progrès et travailler ensemble est le succès</p>,
    "footerQuoteGr": <p>Zusammenkommen ist ein Anfang, Zusammenbleiben ist Fortschritt und Zusammenarbeiten ist Erfolg</p>,

//PAGENAVBAR
    "pageNavbarOneEn": "Profile",
    "pageNavbarOneFr": "Profil",
    "pageNavbarOneGr": "Profil",

    "pageNavbarTwoEn": "Experience",
    "pageNavbarTwoFr": "Expérience",
    "pageNavbarTwoGr": "Erfahrung",

    "pageNavbarThreeEn": "Abilities",
    "pageNavbarThreeFr": "Compétences",
    "pageNavbarThreeGr": "Können",

    "pageNavbarFourEn": "Projects",
    "pageNavbarFourFr": "Projets",
    "pageNavbarFourGr": "Projekte",

    "pageNavbarFiveEn": "Interest",
    "pageNavbarFiveFr": "Interêt",
    "pageNavbarFiveGr": "Interesse",

    "pageNavbarSixEn": "Contact",
    "pageNavbarSixFr": "Contact",
    "pageNavbarSixGr": "Kontakt",

//PROFILE SECTION
    //ABOUT
    "aboutTitleEn": "About",
    "aboutTitleFr": "Bio",
    "aboutTitleGr": "Bio",

    "aboutTextEn": <p>  Unusual, economist 🏦 📈 and full-stack web developer 🖥️ ⌨️ 
                        Curious to always learn more, eager for challenges 
                        and ready for more after the extra mile 🚨 🚀
                        <br/>
                        <br/>
                        I am proactive, enterprising and I love teamwork.
                        "If everyone is moving forward together, then success takes care of itself." – Henry Ford 
                    </p>,
    "aboutTextFr": <p>  Economiste 🏦 📈 et développeur fullstack 🖥️ ⌨️ inaccoutumé,
                        je suis curieux pour apprendre toujours plus, avide de défis 
                        et prêt pour me donner plus après l'effort supplémentaire 🚨 🚀
                        <br/>
                        <br/>
                        je suis proactif, enterprenant et je suis fait pour le travail en équipe.
                        "Si tout le monde avance ensemble, alors le succès prend soin de lui-même." – Henry Ford 
                    </p>,
    "aboutTextGr": <p>  Ungewöhnlicher, ökonomischer 🏦 📈 und Full-Stack-Webentwickler 🖥️ ⌨️ 
                        Neugierig, immer mehr zu lernen, eifrig auf Herausforderungen 
                        und bereit für mehr nach der Extrameile 🚨 🚀
                        <br/>
                        <br/>
                        Ich bin proaktiv, unternehmungslustig und ich liebe Teamarbeit.
                        "Wenn alle gemeinsam vorankommen, kümmert sich der Erfolg um sich." – Henry Ford 
                    </p>,

    //DETAILS
    "detailsTitleEn": "Details",
    "detailsTitleFr": "Etat civil",
    "detailsTitleGr": "Einzelheiten",

    "nameTitleEn": "Name",
    "nameTitleFr": "Nom",
    "nameTitleGr": "Name",

    "nicknameTitleEn": "Nickname",
    "nicknameTitleFr": "Surnom",
    "nicknameTitleGr": "Spitzname",

    "ageTitleEn": "Age",
    "ageTitleFr": "Age",
    "ageTitleGr": "Alter",

    "adressTitleEn": "Adress",
    "adressTitleFr": "Adresse",
    "adressTitleGr": "Anschrift",

    "adressTextEn": "France, Europe",
    "adressTextFr": "France, Europe",
    "adressTextGr": "Frankreich, Europa",

    //FINAL Line
    "finalLineEn": <p>
                        A must-know about me is my love for flags.
                        If you think you are able to offer me a 60x35" flag 
                        or less big of your country my adress is found just above!
                    </p>,
    "finalLineFr": <p>
                        Mon grand amour pour les drapeaux est une des caractéristiques de ma personnalité. 
                        Si vous pensez pouvoir m'offir un drapeau 150x90 cm ou moins grand de votre pays,
                        mon adresse se retrouve juste au-dessus !
                    </p>,
    "finalLineGr": <p>
                        Ein Muss über mich ist meine Liebe zu Fahnen.
                        Wenn Sie denken, dass Sie mir eine 150x90 cm Flagge anbieten können
                        oder weniger groß von deinem Land meine Anschrift ist gleich oben zu finden!
                    </p>,

//EXPERIENCE SECTION
    // EDUCATION
    "educationTitleEn": "Education",
    "educationTitleFr": "Etudes supérieurs",
    "educationTitleGr": "Ausbildung",
    
    "educationOneLocationEn": "Grenoble-Alpes University - France",
    "educationOneLocationFr": "Université Grenoble-Alpes",
    "educationOneLocationGr": "Universität Grenoble-Alpes",

    "educationOnePurposeEn": "Master in mathematics and computer science",
    "educationOnePurposeFr": "Master 2 en mathématiques et informatique",
    "educationOnePurposeGr": "Master Mathematik und Informatik",

    "educationOneDescriptionEn": "This coming school year I will be, mainly, learnig and putting in practice the following: computer systems and networks, software engineering, SQL databases, PHP, JAVA EE, DOT NET",
    "educationOneDescriptionFr": "Cette année universitaire, je vais principalement apprendre et mettre en pratique les éléments suivants: systèmes et réseaux informatiques, génie logiciel, bases de données SQL, PHP, JAVA EE, DOT NET",
    "educationOneDescriptionGr": "Im kommenden Schuljahr werde ich hauptsächlich lernen und in die Praxis umsetzen: Computersysteme und Netzwerke, Software-Engineering, SQL-Datenbanken, PHP, JAVA EE, DOT NET",

    "educationTwoLocationEn": <p>Grenoble-Alpes University - France <br /> & Online courses</p>,
    "educationTwoLocationFr": <p>Université Grenoble-Alpes - France <br /> & cours en ligne</p>,
    "educationTwoLocationGr": <p>Universität Grenoble-Alpes - Frankreich <br /> & Online-Kurse</p>,

    "educationTwoPurposeEn": "English, German & web development courses",
    "educationTwoPurposeFr": "Cours d'anglais, d'allemand et de développement web",
    "educationTwoPurposeGr": "Englisch, Deutsch & Webentwicklungskurse",

    "educationTwoDescriptionEn": "These years have been invested in learning English (C1 level reached), and German (level A2 reached) at the university and web development, that became myfirst passion, by taking online courses. Udemy, W3Schools and openClassrooms are the main educational platforms behind the skills listed in the abilities section below.",
    "educationTwoDescriptionFr": "Ces années ont été investies dans l'apprentissage de l'anglais (niveau C1 atteint), et de l'allemand (niveau A2 atteint) à l'université et du développement Web, qui est devenu ma première passion, en suivant des cours en ligne. Udemy, W3Schools et openClassrooms sont les principales plates-formes éducatives derrière les compétences énumérées dans la section correspondante ci-dessous.",
    "educationTwoDescriptionGr": "Diese Jahre wurden in das Erlernen von Englisch (Stufe C1 erreicht) und Deutsch (Stufe A2 erreicht) an der Universität und in die Webentwicklung investiert, die zu meiner ersten Leidenschaft wurden, indem Online-Kurse besucht wurden. Udemy, W3Schools und openClassrooms sind die wichtigsten Bildungsplattformen für die im entsprechenden Abschnitt aufgeführten Fähigkeiten.",

    "educationThreeLocationEn": "Grenoble-Alpes University - France",
    "educationThreeLocationFr": "Université Grenoble-Alpes",
    "educationThreeLocationGr": "Universität Grenoble-Alpes",

    "educationThreePurposeEn": "Master in energy economics",
    "educationThreePurposeFr": "Master en économie de l'énergie",
    "educationThreePurposeGr": "Master in Energiewirtschaft",

    "educationThreeDescriptionEn": "My knowledge has been expaded to the renewalbe energy and electricity markets, the environmental economics and I have deepened my knowlege of energy accounting, finance, trading and investments",
    "educationThreeDescriptionFr": "Mes connaissances ont été étendues aux marchés de l’énergie renouvelable et de l’électricité, à l’économie de l’environnement et j’ai pu approfondir mes connaissances de comptabilité énergétique, de la finance, du trading et des investissements.",
    "educationThreeDescriptionGr": "Mein Wissen wurde auf die Märkte für erneuerbare Energien und Strom sowie auf die Umweltökonomie ausgeweitet und ich habe meine Kenntnisse in den Bereichen Energiebuchhaltung, Finanzen, Handel und Investitionen vertieft.",

    "educationFourLocationEn": "M'Hamed-Bougara University - Algeria",
    "educationFourLocationFr": "Université M'Hamed-Bougara - Algérie",
    "educationFourLocationGr": "Universität M'Hamed-Bougara - Algerien",

    "educationFourPurposeEn": "Master in energy economics",
    "educationFourPurposeFr": "Master en économie de l'énergie",
    "educationFourPurposeGr": "Master in Energiewirtschaft",

    "educationFourDescriptionEn": "This program led me to master the fondamentals of energy economics: energy markets, fincance, trading, project management and feasibility study. Besides, the program included marketing, general and cost accounting, advanced statistics and more. As a last note, I got the first taste for programming as I learned mainly C++",
    "educationFourDescriptionFr": "Ce programme m'a amené à maîtriser les fondamentaux de l'économie énergétique: marchés de l'énergie, finance, trading, gestion de projet et étude de faisabilité. En outre, le programme comprenait le marketing, la comptabilité générale et analytique, les statistiques avancées et plus encore. C'est aussi ici que j'ai eu le premier goût pour la programmation, principalement en C ++.",
    "educationFourDescriptionGr": "Dieses Programm brachte mich dazu, die Grundlagen der Energiewirtschaft zu beherrschen: Energiemärkte, Finanzen, Handel, Projektmanagement und Machbarkeitsstudie. Außerdem umfasste das Programm Marketing, allgemeine und Kostenrechnung, erweiterte Statistiken und vieles mehr. Als letzte Anmerkung bekam ich den ersten Einblick in das Programmieren, da ich hauptsächlich C ++ lernte",

    "educationFiveLocationEn": "The New High school - Constantine, Algeria",
    "educationFiveLocationFr": "Le Nouveau Lycée - Constantine, Algérie",
    "educationFiveLocationGr": "Die neue High School - Constantine, Algerien",

    "educationFivePurposeEn": "High school diploma",
    "educationFivePurposeFr": "Baccalauréat",
    "educationFivePurposeGr": "Abitur",

    "educationFiveDescriptionEn": "Here I was first introduced to computer science basics and I developed an interest for economics, finance and the world of energy",
    "educationFiveDescriptionFr": "C'est ici que j'ai d'abord découvert les bases de l'informatique et que j'ai développé un intérêt pour l'économie, la finance et le monde de l'énergie.",
    "educationFiveDescriptionGr": "Hier lernte ich zunächst die Grundlagen der Informatik kennen und entwickelte ein Interesse für Wirtschaft, Finanzen und die Welt der Energie",

    //CAREER
    "careerTitleEn": "Career",
    "careerTitleFr": "Expérience professionnelle",
    "careerTitleGr": "Berufserfahrung",

    "careerOneLocationEn": "The French National Center for Scientific Research",
    "careerOneLocationFr": "Le Centre national de la recherche scientifique",
    "careerOneLocationGr": "Das Nationale Zentrum für wissenschaftliche Forschung",

    "careerOneDateEn": "June 2015 - September 2015",
    "careerOneDateFr": "Juin 2015 - Septembre 2015",
    "careerOneDateGr": "Juni 2015 - September 2015",

    "careerOnePurposeEn": "Research internship",
    "careerOnePurposeFr": "Stage de recherche",
    "careerOnePurposeGr": "Forschungspraktikum",

    "careerOneDescriptionEn": "The experience was a success in helping the research center find the best way to regulate and finance the new smart meters for the smart electricity grids. I had to do a benchmark studying the markets and policy of advanced countries in the matter, such as Sweden and Australia, to help France move forward",
    "careerOneDescriptionFr": "Cette expérience a permis au centre de recherche de trouver le meilleur moyen de réglementer et de financer les nouveaux compteurs intelligents pour les réseaux électriques intelligents. Je devais faire un benchmark en étudiant les marchés et la politique des pays avancés en la matière, tels que la Suède et l'Australie, pour aider la France à progresser.",
    "careerOneDescriptionGr": "Die Erfahrung war ein Erfolg, der dem Forschungszentrum dabei half, den besten Weg zur Regulierung und Finanzierung der neuen intelligenten Zähler für die intelligenten Stromnetze zu finden. Ich musste ein Benchmarking durchführen, um die Märkte und die Politik der Industrieländer wie Schweden und Australien zu untersuchen und Frankreich dabei zu unterstützen, voranzukommen",

    "careerTwoLocationEn": "Sonatrach, the large Algerian oil consortium",
    "careerTwoLocationFr": "Sonatrach, le grand consortium pétrolier algérien",
    "careerTwoLocationGr": "Sonatrach, das große algerische Ölkonsortium",

    "careerTwoDateEn": "March 2014 - June 2014",
    "careerTwoDateFr": "Mars 2014 - Juin 2014",
    "careerTwoDateGr": "März 2014 - Juine 2014",

    "careerTwoPurposeEn": "Internship",
    "careerTwoPurposeFr": "Stage de professionnalisation",
    "careerTwoPurposeGr": "Professionalisierungspraktikum",

    "careerTwoDescriptionEn": "A great first experince in the world of business in the largest company in Africa. My energy economics knowledge has been put into pratice through a feasibility study (some technical and mainly economic) of a large real life investment project that aimed to increase the gas production of a group of wells",
    "careerTwoDescriptionFr": "Une grande première expérience dans le monde professionnel dans la plus grande entreprise en Afrique. Mes connaissances en économie de l'énergie ont été mises en pratique par le biais d'une étude de faisabilité (à la fois technique et principalement économique) d'un grand projet d'investissement visant à augmenter la production de gaz d'un groupe de puits.",
    "careerTwoDescriptionGr": "Eine großartige erste Erfahrung in der Berufswelt im größten Unternehmen Afrikas. Meine Kenntnisse der Energiewirtschaft wurden durch eine Machbarkeitsstudie (sowohl technisch als auch hauptsächlich wirtschaftlich) eines großen Investitionsprojekts umgesetzt, das darauf abzielte, die Gasproduktion einer Gruppe von Bohrlöchern zu steigern",

    //ACTIVITIES
    "activitiesTitleEn": "Extracurricular activities",
    "activitiesTitleFr": "Expérience extra-professionnelle",
    "activitiesTitleGr": "Außergewöhnliche Erfahrung",

    "activityOneLocationEn": "Student Remix Association - Grenoble, France",
    "activityOneLocationFr": "Association Remix Etudiants - Grenoble, France",
    "activityOneLocationGr": "Studentenverein Remix - Grenoble, Frankreich",

    "activityOneDateEn": "Half-time volonteer",
    "activityOneDateFr": "Volontaire à mi-temps",
    "activityOneDateGr": "Halbzeit Volonteer",

    "activityOnePurposeEn": "Logistics team leader and association's secretary",
    "activityOnePurposeFr": "Chef d'équipe logistique et secrétaire de l'association",
    "activityOnePurposeGr": "Leiter des Logistikteams und Sekretär des Verbandes",

    "activityOneDescriptionEn": "Along with a team we took care of the maintenance of the 200m² association premises while I was in charge of the administrative part to handle the members and paperwork.",
    "activityOneDescriptionFr": "En équipe, nous nous sommes occupés de l'entretien des locaux de l'association, d'une superficie de 200 m², tandis que j'étais responsable de la partie administrative, chargée de la gestion des membres et du traitement des documents.",
    "activityOneDescriptionGr": "Als Team kümmerten wir uns um die Instandhaltung der Räumlichkeiten des Vereins mit einer Fläche von 200 m², während ich für den administrativen Teil verantwortlich war, der für die Verwaltung der Mitglieder und die Dokumentenverarbeitung verantwortlich war.",

    "activityTwoLocationEn": "IKEA - Grenoble, France",
    "activityTwoLocationFr": "IKEA - Grenoble, France",
    "activityTwoLocationGr": "IKEA - Grenoble, Frankreich",

    "activityTwoDateEn": "4 months full-time job",
    "activityTwoDateFr": "4 mois de travail à temps plein",
    "activityTwoDateGr": "4 Monate Vollzeitjob",

    "activityTwoPurposeEn": "Restaurant employee",
    "activityTwoPurposeFr": "Employé de restaurant",
    "activityTwoPurposeGr": "Restaurantangestellter",

    "activityTwoDescriptionEn": "In the world-famous IKEA store I participated with many colleagues in the preparation and service of Swedish dishes to a wonderful clientele",
    "activityTwoDescriptionFr": "Dans le magasin de renommée mondiale IKEA, j'ai participé avec de nombreux collègues à la préparation et le service de plats suédois à une clientèle formidable.",
    "activityTwoDescriptionGr": "Im weltberühmten IKEA-Laden habe ich mit vielen Kollegen an der Zubereitung und dem Servieren schwedischer Gerichte für eine wunderbare Kundschaft teilgenommen.",

    "activityThreeLocationEn": "University café and library - Grenoble, France",
    "activityThreeLocationFr": "Café et bibliothèque universitaires  - Grenoble, France",
    "activityThreeLocationGr": "Universitätscafé und Bibliothek - Grenoble, Frankreich",

    "activityThreeDateEn": "Half-time job",
    "activityThreeDateFr": "Travail à mi-temps",
    "activityThreeDateGr": "Halbzeitjob",

    "activityThreePurposeEn": "Café employee and library manager assistant",
    "activityThreePurposeFr": "Employé de café et assistant de directeur de bibliothèque",
    "activityThreePurposeGr": "Mitarbeiter im Café und Assistent des Bibliotheksleiters",

    "activityThreeDescriptionEn": "I was honored to serve other students both in a café as a server and cashier and in a library as managment assistant (handle the books, printers, computers...)",
    "activityThreeDescriptionFr": "J'ai eu l'honneur de servir d'autres étudiants dans un café en tant que serveur et caissier, ainsi que dans une bibliothèque en tant qu'assistant de gestion (gérer les livres, les imprimantes, les ordinateurs, etc.).",
    "activityThreeDescriptionGr": "Es war mir eine Ehre, anderen Studenten sowohl in einem Café als Server und Kassierer als auch in einer Bibliothek als Verwaltungsassistent zu dienen (Umgang mit Büchern, Druckern, Computern ...)",

    "activityFourLocationEn": "RGIS (international inventory company) - Grenoble, France",
    "activityFourLocationFr": "RGIS (société internationale d'inventaire) - Grenoble, France",
    "activityFourLocationGr": "RGIS (internationales Inventarunternehmen) - Grenoble, Frankreich",

    "activityFourDateEn": "Half-time job",
    "activityFourDateFr": "Travail à mi-temps",
    "activityFourDateGr": "Halbzeitjob",

    "activityFourPurposeEn": "Inventory team leader",
    "activityFourPurposeFr": "Chef d'équipe d'inventaire",
    "activityFourPurposeGr": "Inventar Teamleiter",

    "activityFourDescriptionEn": "As a team leader (of 3 to 20 people) I was flatterd every evening employees were looking forward to be in my team again. That was a sign of the success of my misson and my leadership talent, and the main reason of my satisfaction",
    "activityFourDescriptionFr": "En tant que chef d'équipe (de 3 à 20 personnes), j'étais flatté tous les soirs où les employés étaient impatients de faire partie de mon équipe. C’était le signe du succès de ma mission et de mon talent en leadership, et la raison principale de ma satisfaction",
    "activityFourDescriptionGr": "Als Teamleiter (von 3 bis 20 Personen) fühlte ich mich jeden Abend geschmeichelt, dass die Mitarbeiter Teil meines Teams sein wollten. Es war ein Zeichen für den Erfolg meiner Mission und meines Führungstalents und der Hauptgrund für meine Zufriedenheit",

// ABILITIES SECTION

    // SKILLS
    "skillsTitleEn": "Skills",
    "skillsTitleFr": "Habiletés",
    "skillsTitleGr": "Geschicke",

    "skillOneEn": "Languages",
    "skillOneFr": "Langages",
    "skillOneGr": "Sprachen",

    "skillTwoEn": "JS runtime & libraries",
    "skillTwoFr": "Environnement d'exécution et bibliothèques JS",
    "skillTwoGr": "JavaScript-Laufzeitumgebung und Bibliotheken",

    "skillThreeEn": "JS frameworks",
    "skillThreeFr": "frameworks JS",
    "skillThreeGr": "JS Frameworks",

    "skillFourEn": "CSS frameworks",
    "skillFourFr": "frameworks CSS",
    "skillFourGr": "CSS Frameworks",

    "skillFiveEn": "Site generators",
    "skillFiveFr": "Générateurs de sites",
    "skillFiveGr": "Website-Generatoren",

    "skillSixEn": "dev platform",
    "skillSixFr": "Plate-formes de développement",
    "skillSixGr": "Entwicklerplattform",

    "skillSevenEn": "Template engines",
    "skillSevenFr": "Gestionnaires de templates",
    "skillSevenGr": "Template-Engine",

    "skillEightEn": "CMS",
    "skillEightFr": "SGC",
    "skillEightGr": "CMS",

    "skillNineEn": "Software",
    "skillNineFr": "Logiciel",
    "skillNineGr": "Software",

    "skillTenEn": "Other",
    "skillTenFr": "Autre",
    "skillTenGr": "Andere",

    // TOOLS
    "toolsTitleEn": "Tools",
    "toolsTitleFr": "Outils",
    "toolsTitleGr": "Werkzeug",

    "toolOneEn": "Code editors",
    "toolOneFr": "Editeurs de code",
    "toolOneGr": "Quelltext-Editoren",

    "toolTwoEn": "DevTools",
    "toolTwoFr": "Outils de développement",
    "toolTwoGr": "DevTools",

    "toolThreeEn": "Version control",
    "toolThreeFr": "Gestion de versions",
    "toolThreeGr": "Versionsverwaltung ",

    "toolFourEn": "JS compiler & bundler",
    "toolFourFr": "Compilateur JS et module bundler",
    "toolFourGr": "JavaScript-Compiler und Modul-Packer",

    "toolFiveEn": "Package managers",
    "toolFiveFr": "Gestionnaires de paquets",
    "toolFiveGr": "Paketmanager",

    "toolSixEn": "Databases",
    "toolSixFr": "Bases de données",
    "toolSixGr": "Datenbanken",

    "toolSevenEn": "ODM, ORM & Prisma",
    "toolSevenFr": "ODM, ORM & Prisma",
    "toolSevenGr": "ODM, ORM & Prisma",

    "toolEightEn": "MS Office",
    "toolEightFr": "MS Office",
    "toolEightGr": "MS Office",

    "toolNineEn": "Mailing",
    "toolNineFr": "Service de livraison d'e-mail",
    "toolNineGr": "E-Mail-Versand",

    "toolTenEn": "Payment platforms",
    "toolTenFr": "Plateformes de paiement",
    "toolTenGr": "Zahlungsplattformen",

    // LANGAUGES
    "langTitleEn": "Languages",
    "langTitleFr": "Langues",
    "langTitleGr": "Sprachen",

    "langOneEn": "English",
    "langOneFr": "Anglais",
    "langOneGr": "Englisch",

    "langTwoEn": "French",
    "langTwoFr": "Français",
    "langTwoGr": "Französisch",

    "langThreeEn": "Arabic",
    "langThreeFr": "Arabe",
    "langThreeGr": "Arabisch",

    "langFourEn": "German-(4 stars by 2021)",
    "langFourFr": "Allemand-(4 étoiles d'ici 2021)",
    "langFourGr": "Deutsch-(4 Sterne bis 2021)",

    "langFiveEn": "Spanish-(Plans of the futur)",
    "langFiveFr": "Espagnol-(Dans les plans du futur)",
    "langFiveGr": "Spanisch-(Plänen der Zukunft)",

    "langSixEn": "Italian-(Plans of the future)",
    "langSixFr": "Italien-(Dans les plans du futur)",
    "langSixGr": "Italienisch-(Plänen der Zukunft)",

    // PROJECTS SECTION
    "projectOneTitleEn": "Interactive resume",
    "projectOneTitleFr": "CV interactif",
    "projectOneTitleGr": "Interaktiv Lebenslauf",

    "projectOneDescEn": "An online interactive resume",
    "projectOneDescFr": "CV interactif",
    "projectOneDescGr": "Interaktiv Lebenslauf",

    "projectTagsTitleEn": "Tags",
    "projectTagsTitleFr": "Mots clés",
    "projectTagsTitleGr": "Stichworte",

    "projectTwoDescEn": "A virtual retail agency",
    "projectTwoDescFr": "Une agence de vente virtuelle",
    "projectTwoDescGr": "Eine virtuelle Einzelhandelsagentur",

    "projectThreeDescEn": "A virtual healthy food place",
    "projectThreeDescFr": "Un lieu virtuel de restauration saine",
    "projectThreeDescGr": "Ein virtueller Ort der gesunden Ernährung",

    "projectFourDescEn": "Expenses app",
    "projectFourDescFr": "Application des dépenses",
    "projectFourDescGr": "Kosten App",

    "projectFiveDescEn": "Recipes app",
    "projectFiveDescFr": "Application de recettes",
    "projectFiveDescGr": "Rezepte App",

    "projectSixDescEn": "A chat app",
    "projectSixDescFr": "Application de chat",
    "projectSixDescGr": "Chat-App",

    // INTEREST SECTION
    "interestOneTitleEn": "Travelling",
    "interestOneTitleFr": "Voyager",
    "interestOneTitleGr": "Reisen",

    "interestOneTextEn": <p>
                            I love visiting new cities. 
                            I've already been to big ones such as Copenhagen, Paris, Milano and Madrid. 
                            Usually in a group, I enjoy discovering new cultures and opening my mind.
                            <br />
                            <br />
                            One of the cities I would love to visit, and why not live in, is London. 
                            Being a fan of football and especially of Chelsea Football Club, one of my favorite 
                            hobbies is to cheer that one
                            <br />
                            <br />
                            Switzerland and Germany are other places I would love to settle in. 
                            I find the language and the culture very attractive
                         </p>,
    "interestOneTextFr": <p>
                            J'aime visiter de nouvelles villes.
                            Je suis déjà allé dans des grandes villes telles que Copenhague, Paris, Milan et Madrid.
                            Habituellement, en groupe, j'aime découvrir de nouvelles cultures et ouvrir mon esprit.
                            <br />
                            <br />
                            Une des villes que j'aimerais visiter et pourquoi pas y vivre est Londres.
                            Etant fan de foot et en particulier de "Chelsea", un de mes passe-temps préférés
                            est regarder les matchs de cette équipe
                            <br />
                            <br />
                            La Suisse et l'Allemagne sont d'autres endroits dans lesquels j'aimerais m'installer.
                            Je trouve la langue et la culture très attrayantes
                         </p>,
    "interestOneTextGr": <p>
                            Ich liebe es, neue Städte zu besuchen.
                            Ich war schon in großen Städten wie Kopenhagen, Paris, Mailand und Madrid.
                            Normalerweise genieße ich es, in einer Gruppe neue Kulturen zu entdecken und meinen Geist zu öffnen.
                            <br />
                            <br />
                            Eine der Städte, die ich besuchen möchte und warum nicht dort leben, ist London.
                            Fußballfan zu sein und vor allem "Chelsea", 
                            eines meiner Lieblingshobbys ist es, die Spiele dieser Mannschaft zu sehen
                            <br />
                            <br />
                            Die Schweiz und Deutschland sind andere Orte, an denen ich mich gerne niederlassen würde.
                            Ich finde die Sprache und die Kultur sehr attraktiv
                         </p>,

    "interestTwoTitleEn": "Volunteering",
    "interestTwoTitleFr": "Faire du bénévolat",
    "interestTwoTitleGr": "Freiwilligenarbeit",

    "interestTwoTextEn": <p>
                            One of my other interests is to volonteer as a helper for people around. 
                            I've spent most of my free time with the Remix Student Association
                            in Grenoble participating into the adjustment of students to their new city, 
                            organising envents and parties for cultural exchange.
                            <br />
                            <br />
                            I've also got the opportunity to help refugees settle in the city, 
                            answering their needs for two years. I've taught them French, computer science, 
                            helped them with paperwork, translation and finding jobs. 
                            It's a great honor for me to see them moved forward since they got here
                         </p>,
    "interestTwoTextFr": <p>
                            Un de mes autres intérêts est de faire du bénévolat pour aider les gens autour.
                            J'ai passé la majeure partie de mon temps libre avec l'association des étudiants Remix 
                            de Grenoble à participer à l'adaptation des étudiants à leur nouvelle ville, 
                            à organiser des fêtes et des échanges culturels.
                            <br />
                            <br />
                            J'ai également eu l'opportunité d'aider les réfugiés à s'installer dans la ville 
                            et à répondre à leurs besoins pendant deux ans. Je leur ai appris le français, 
                            l'informatique, et je les ai aidés avec la paperasse, la traduction et la recherche d'emploi.
                            C'est un grand honneur pour moi de les voir progresser depuis leur arrivée ici.
                         </p>,
    "interestTwoTextGr": <p>
                            Eines meiner anderen Interessen ist es, mich als Helfer für die Menschen in 
                            der Umgebung zur Verfügung zu stellen.
                            Ich habe die meiste Zeit meiner Freizeit mit der Remix Student Association in Grenoble 
                            verbracht und mich an der Anpassung von Studenten an ihre neue Stadt beteiligt 
                            und Veranstaltungen und Partys für den kulturellen Austausch organisiert.
                            <br />
                            <br />
                            Ich hatte auch die Möglichkeit, Flüchtlingen zu helfen, 
                            sich in der Stadt niederzulassen und ihre Bedürfnisse für zwei Jahre zu befriedigen. 
                            Ich unterrichtete sie in Französisch, Informatik und half ihnen bei Papierkram, 
                            Übersetzung und Jobsuche. Es ist mir eine große Ehre, dass sie seit ihrer Ankunft hier 
                            Fortschritte gemacht haben.
                         </p>,

    "interestThreeTitleEn": "Movies",
    "interestThreeTitleFr": "Cinéma",
    "interestThreeTitleGr": "Kino",

    "interestThreeTextEn": <p>
                                I am also a big fan of the cinema world. 
                                My favorite TV shows are: 
                                "Breaking Bad", "The Big Bang Theory", "Suits", "La Casa De Papel", "Prison Break", "HIMYM" and "Chuck" 
                                (I couldn't leave one of them off the list)
                                <br />
                                <br />
                                My top three movies are: 
                                "The Hobbit", "The Lord Of The Rings" and "Titanic" (We wouldn't say but yes!) 
                           </p>,
    "interestThreeTextFr": <p>
                                Je suis aussi un grand fan du monde du cinéma.
                                Mes séries télévisées préférées sont:
                                "Breaking Bad", "Big Bang Théorie", "Suits", "La Casa De Papel", "Prison Break", "HIMYM" et "Chuck"
                                (Je ne pouvais pas en laisser un en dehors de la liste)
                                <br />
                                <br />
                                Mes trois meilleurs films sont:
                                "Le Hobbit", "Le Seigneur des Anneaux" et "Titanic" (On ne dirait pas mais oui!) 
                           </p>,
    "interestThreeTextGr": <p>
                                Ich bin auch ein großer Fan der Kinowelt.
                                Meine Lieblingsfernsehsendungen sind:
                                "Breaking Bad", "The Big Bang Theory", "Suits", "La Casa De Papel", "Prison Break", "HIMYM" und "Chuck"
                                (Ich konnte keinen von der Liste lassen)
                                <br />
                                <br />
                                Meine drei besten Filme sind:
                                "Der Hobbit", "Der Herr der Ringe" und "Titanic" (Wir würden nicht sagen, aber ja!) 
                           </p>,

    "interestFourTitleEn": "Sports",
    "interestFourTitleFr": "Sports",
    "interestFourTitleGr": "Sporte",

    "interestFourTextEn": <p>
                               Besides that, I would be watching football games
                               playing football on video games or practicing football in real life 
                               (That's a lot of football but I also enjoy the gym and swimming)
                          </p>,
    "interestFourTextFr": <p>
                               En plus de cela, je regarderais bien des matchs de football, 
                               jouerais au football sur des jeux vidéo ou pratiquerais le football dans la vie réelle
                               (Cela fait beaucoup de football mais j'aime aussi la gym et la natation)
                          </p>,
    "interestFourTextGr": <p>
                               Außerdem schaue ich mir Fußballspiele an, spiele Fußball bei Videospielen 
                               oder übe Fußball im wirklichen Leben 
                               (das ist viel Fußball, aber ich mag auch das Fitnessstudio und das Schwimmen)
                          </p>,

    "interestFiveTitleEn": "Web development",
    "interestFiveTitleFr": "Développement web",
    "interestFiveTitleGr": "Web Entwicklung",

    "interestFiveTextEn": <p>
                               Last but far from least I find programming fun, challenging and captivating. 
                               So I meant to be a web developer
                          </p>,
    "interestFiveTextFr": <p>
                               Enfin, je trouve que la programmation est amusante, stimulante et captivante. 
                               Donc je suis fait pour être un développeur web
                          </p>,
    "interestFiveTextGr": <p>
                               Schließlich finde ich, dass das Programmieren Spaß macht, anregend und fesselnd ist.
                               Also bin ich zum Webentwickler gemacht
                          </p>,
}

export default data